...
function generatePixQRCode(pixData: any): string {
  const {
    pixKey,
    pixAmount,
    pixDescription,
  ...
  const pixCode = `00020126${pixKey.length.toString().padStart(2, '0')}${pixKey}5204000053039865406${pixAmount.toFixed(2).padStart(10, '0')}5925${pixRecipientName.padEnd(25, ' ').substring(0, 25)}6009${pixCity.padEnd(15, ' ').substring(0, 15)}62${pixDescription.length.toString().padStart(2, '0')}${pixDescription}6304`;
              ...
              <div className="bg-white rounded-lg p-6 border-2 border-green-200 shadow-lg">
                <div className="text-center">
                  ...
                  <h3 className="text-xl font-bold text-gray-900 mb-2">
                    {element.pixTitle || "Pagamento PIX"}
                  </h3>
                  
                  <p className="text-gray-600 mb-4">
                    {element.pixDescription || "Escaneie o QR Code ou copie o código PIX"}
                  </p>
                  
                  <div className="mb-4">
                    <div className="bg-green-50 p-4 rounded-lg inline-block">
                      <span className="text-2xl font-bold text-green-600">
                        R$ {element.pixAmount ? Number(element.pixAmount).toFixed(2) : "0,00"}
                      </span>
                    </div>
                  </div>
                      ...
                      <div className="text-center">
                        <img 
                          src={generatePixQRCode({
                            pixKey: element.pixKey,
                            pixAmount: element.pixAmount || 0,
                            pixDescription: element.pixDescription || "Pagamento",
                            pixRecipientName: element.pixRecipientName || "Vendedor",
                            pixCity: element.pixCity || "São Paulo"
                          })}
                          alt="QR Code PIX"
                  ...
                  <div className="space-y-3">
                    <div className="bg-gray-50 p-3 rounded-lg">
                      ...
                      <div className="flex items-center justify-between bg-white p-2 rounded border">
                        <span className="text-sm text-gray-600 truncate">
                          {element.pixKey || "chave@exemplo.com"}
                        </span>
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => {
                            navigator.clipboard.writeText(element.pixKey || "");
                    ...
                    <Button
                      ...
                      onClick={() => {
                        // Ação do botão PIX
                        console.log("Pagamento PIX iniciado");
                      }}
                    >
                      PAGAR NO PIX
                    </Button>
                  ...
                  <div className="mt-4 flex items-center justify-center text-xs text-gray-500">
                    ...
                    <span>Confirme o pagamento em até 10 minutos</span>
                  </div>
                </div>
              </div>
            </div>
