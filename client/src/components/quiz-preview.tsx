  ...
  const renderContentElement = (element: any) => {
    switch (element.type) {
      ...
      case 'share_quiz':
        ...
        // Função para gerar URL correta do quiz (preview mode)
        const getCurrentQuizUrl = () => {
          // No preview, vamos usar uma URL fictícia para demonstração
          const baseUrl = window.location.origin;
          const quizId = quiz?.id || 'preview-quiz';
          return `${baseUrl}/quiz/${quizId}`;
        };

        const quizUrl = getCurrentQuizUrl();
        const shareMessage = element.shareMessage || 'Faça esse teste e se surpreenda também!';
        ...
        const handleShare = (network: string) => {
          ...
          switch (network) {
            ...
            case 'instagram':
              ...
              toast({
                title: "Link copiado!",
                description: "Link copiado para área de transferência! Cole no Instagram.",
              });
              return;
            case 'email':
        ...
        return (
          <div className="space-y-4 p-4 bg-transparent mb-6">
