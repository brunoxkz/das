...
export function registerCheckoutRoutes(app: Express) {
  ...
  app.post("/api/quiz-ia/create", verifyJWT, async (req: any, res) => {
    try {
      ...
      const { quizData, generatedContent, pixKey } = req.body;

      // Validar dados
      if (!quizData || !generatedContent || !pixKey) {
        return res.status(400).json({ 
          success: false, 
          error: "Dados incompletos para criar o quiz" 
        });
      }

      console.log(`🎯 QUIZ I.A.: Criando quiz final para "${quizData.productName}"`);

      // Estrutura do quiz baseada no conteúdo gerado
      const quizStructure = {
        pages: generatedContent.questions.map((question: any, index: number) => ({
          ...
          elements: [{
            ...
            placeholder: question.placeholder || "",
            required: true,
            fieldId: question.type === 'text' ? 'nome' : 
          ...
          // Página de transição
          {
            ...
            elements: [{
              id: "transition-text",
              type: "text_display",
              content: generatedContent.transitions.goodNews
            }]
          },
          // Página de pitch/checkout
          {
            id: "checkout-page", 
            title: "Sua Solução Exclusiva",
            elements: [{
              id: "pitch-content",
              type: "text_display", 
              content: generatedContent.transitions.pitch
            }, {
              id: "checkout-button",
              type: "pix_checkout",
              ...
              pixKey: pixKey,
              checkoutData: generatedContent.checkout
            }]
          }
        ]),
        settings: {
          title: `Quiz: ${quizData.productName}`,
          description: `Descubra se ${quizData.productName} é ideal para você`,
          theme: "modern",
          progressBar: true,
          pixIntegration: {
            enabled: true,
            pixKey: pixKey,
            productInfo: {
              name: quizData.productName,
              price: parseFloat(quizData.productPrice),
              description: quizData.solution
            }
          }
        }
      };
      ...
      const quiz = {
        id: quizId,
        title: `Quiz I.A.: ${quizData.productName}`,
        description: `Quiz gerado automaticamente para ${quizData.niche}`,
        structure: quizStructure,
        userId: req.user.id,
        isPublished: true,
        createdBy: "quiz-ia",
        pixKey: pixKey,
        productPrice: parseFloat(quizData.productPrice)
      };
      ...
      console.log(`✅ QUIZ I.A.: Quiz criado com sucesso - ID: ${quizId}`);
      console.log(`🔗 URL do quiz: ${quizUrl}`);
      ...
      console.error("❌ Erro ao criar quiz I.A.:", error);
      res.status(500).json({ 
        success: false, 
        error: "Erro interno do servidor" 
      });
