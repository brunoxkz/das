...
function testEndpoint(method, endpoint, data = {}) {
  return new Promise((resolve) => {
    let curlCommand;
    if (method === 'GET') {
      curlCommand = `curl -s -X GET http://localhost:5000${endpoint}`;
    } else {
      curlCommand = `curl -s -X ${method} http://localhost:5000${endpoint} -H "Content-Type: application/json" -d '${JSON.stringify(data)}'`;
    }
...
async function runTests() {
  ...
  const tests = [
    { method: 'POST', endpoint: '/push/vapid', data: {} },
    { method: 'POST', endpoint: '/push/stats', data: {} },
    { method: 'POST', endpoint: '/push/send', data: { title: 'Teste', message: 'Mensagem de teste' } },
    { method: 'POST', endpoint: '/push/subscribe', data: { subscription: { endpoint: 'test', keys: { p256dh: 'test', auth: 'test' } } } }
  ];
